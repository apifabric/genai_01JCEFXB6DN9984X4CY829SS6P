...
resourcename="dessertroadtripplanner"  # lower case, only
resourcegroup="dessertroadtripplanner_rg"
...
# git clone https://github.com/apilogicserver/dessertroadtripplanner.git
# cd dessertroadtripplanner
...
az group create --name dessertroadtripplanner_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group dessertroadtripplanner_rg --sku S1 --is-linux
az container create --resource-group dessertroadtripplanner_rg --name dessertroadtripplanner --image apilogicserver/dessertroadtripplanner:latest --dns-name-label dessertroadtripplanner --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//dessertroadtripplanner.westus.azurecontainer.io:5656'
